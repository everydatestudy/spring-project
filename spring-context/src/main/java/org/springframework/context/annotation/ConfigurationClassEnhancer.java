 ...
 * 
 * CGLIB创建动态代理是基于继承来是实现的(JDK的动态代理是基于接口实现)，
 * 因此enhancer.setSupperclass(configSuperClass)这一行代码，
 * 就是为即将产生的代理对象设置父类，
 * 同时为产生的代理对象实现EnhancedConfiguration.class接口，
 * 实现该接口的目的，是为了该Configuration类在实例化、初始化过程中，
 * 执行相关的BeanPostProcessor。
 * 例如在执行ImportAwareBeanPostProcessor后置处理器时，
